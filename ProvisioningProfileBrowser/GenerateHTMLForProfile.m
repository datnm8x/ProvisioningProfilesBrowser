...
#import "GenerateHTMLForProfile.h"
...
NSMutableString* GenerateHTMLPreviewForURL(CFURLRef url, CFStringRef contentTypeUTI);
...
@implementation GenerateHTMLForProfile

+ (NSString*)generateHTMLPreviewForProfileURL:(NSURL*) url {
  NSMutableString* result = GenerateHTMLPreviewForURL((__bridge CFURLRef)(url), (__bridge CFStringRef)(@"com.apple.mobileprovision"));
  return  result;
}
...
NSDictionary *formattedDevicesData(NSArray *value) {
  ...
  [devices appendString:@"<table style=\"width:100%\">\n"];
  [devices appendString:@"<tr><th style=\"width:30%\" class=\"col_left\"></th><th class=\"col_right\"></th></tr>\n"];

  for (NSString *device in sortedArray) {
    [devices appendFormat:@"<tr><td class=\"col_left\">Device ID</td><td class=\"col_right\">%@</td></tr>\n", device];
  }
...
NSMutableString* GenerateHTMLPreviewForURL(CFURLRef url, CFStringRef contentTypeUTI) {
  @autoreleasepool {
    ...
    NSURL *htmlURL = [[NSBundle mainBundle] URLForResource:@"template" withExtension:@"html"];
    ...
    NSString *pluginBundleId = [[NSBundle mainBundle] bundleIdentifier];
    if (pluginBundleId == nil) {
      pluginBundleId = @"datnm8x.com.vn.ProvisioningProfileBrowser";
    }

    synthesizedValue = [[NSBundle mainBundle] objectForInfoDictionaryKey:@"CFBundleShortVersionString"];
    if (synthesizedValue != nil) {
      [synthesizedInfo setObject:synthesizedValue forKey:@"BundleShortVersionString"];
    }

    synthesizedValue = [[NSBundle mainBundle] objectForInfoDictionaryKey:@"CFBundleVersion"];
    if (synthesizedValue != nil) {
