...
public extension SwiftyModule.ProvisioningProfile {
  var applicationID: String? { entitlements["application-identifier"]?.string }
  var teamID: String { teamIdentifiers.first ?? "" }
  var getTaskAllow: Bool { entitlements["get-task-allow"]?.boolValue ?? false }
  var keychainAccessGroups: [String] { entitlements["keychain-access-groups"]?.arrayValue.compactMap({ $0.string }) ?? [] }
  var apsEnvironment: String? { entitlements["aps-environment"]?.string }
  var securityApplicationGroups: [String] { entitlements["com.apple.security.application-groups"]?.arrayValue.compactMap({ $0.string }) ?? [] }
}
